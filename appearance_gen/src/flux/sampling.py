...
def prepare_single(t5: HFEmbedder, clip: HFEmbedder, img: Tensor, prompt: str | list[str]) -> dict[str, Tensor]:
    bs, c, h, w = img.shape
...
def denoise_controlnet_mix(
    ...
    for t_curr, t_prev in zip(timesteps[:-1], timesteps[1:]):
        t_vec = torch.full((img.shape[0],), t_curr, dtype=img.dtype, device=img.device)
        control = torch.cat([depth,normal,hand,seg],dim=1)
...
def denoise_double_control(
    ...
    for t_curr, t_prev in zip(timesteps[:-1], timesteps[1:]):
        t_vec = torch.full((img.shape[0],), t_curr, dtype=img.dtype, device=img.device)
        physical_latent=torch.cat([depth,normal],dim=1)
        segmantic_latent=torch.cat([hand, seg],dim=1)
        # segmantic_latent = hand
        # default_latent1 = -torch.ones_like(physical_latent,device=physical_latent.device,dtype=physical_latent.dtype)
        # default_latent2 = -torch.ones_like(segmantic_latent,device=segmantic_latent.device,dtype=segmantic_latent.dtype)
        ...
        #             controlnet_cond=default_latent1,
        #             **inp_condition_default,
        #             timesteps=t_vec,
        #             guidance=guidance_vec,
        #         )
        ...
        #             controlnet_cond=default_latent2,
        #             **inp_condition_default,
        #             timesteps=t_vec,
        #             guidance=guidance_vec,
        #         )
        pred = model(
            **inp_condition_true,
            timesteps=t_vec,
            guidance=guidance_vec,
            block_controlnet_hidden_states=[i*control_gs[0]+j*control_gs[1] for i,j in zip(block_res_samples_1,block_res_samples_2)],
        )
    ...
    **kwargs
    ...
    for t_curr, t_prev in zip(timesteps[:-1], timesteps[1:]):
        ...
        pred = model(
            ...
            timesteps=t_vec,
            guidance=guidance_vec,
            block_controlnet_hidden_states=[res for res in block_res_samples],
        )
