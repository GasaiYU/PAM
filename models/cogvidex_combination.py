...
class CogVideoXTransformer3DModelCombination(CogVideoXTransformer3DModel, ModelMixin):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Create linear layers for combining hidden states and conditions
        ...
        # For initial combination of hidden states and conditions (tracking map, depth, normal, seg, hand keypoints)
        self.initial_combine_linear = nn.Linear(inner_dim*5, inner_dim)
    ...
    def forward(
        ...
        # Process normal maps
        prompt_embed = encoder_hidden_states.clone()
        normal_maps_hidden_states = self.patch_embed(prompt_embed, normal_maps)
        normal_maps_hidden_states = self.embedding_dropout(normal_maps_hidden_states)
        del prompt_embed

        # Process depth maps
        prompt_embed = encoder_hidden_states.clone()
        depth_maps_hidden_states = self.patch_embed(prompt_embed, depth_maps)
        depth_maps_hidden_states = self.embedding_dropout(depth_maps_hidden_states)
        del prompt_embed

        # Process segmentation masks
        prompt_embed = encoder_hidden_states.clone()
        seg_masks_hidden_states = self.patch_embed(prompt_embed, seg_masks)
        seg_masks_hidden_states = self.embedding_dropout(seg_masks_hidden_states)
        del prompt_embed

        # Process hand keypoints
        prompt_embed = encoder_hidden_states.clone()
        hand_keypoints_hidden_states = self.patch_embed(prompt_embed, hand_keypoints)
        hand_keypoints_hidden_states = self.embedding_dropout(hand_keypoints_hidden_states)
        del prompt_embed
        ...
        normal_maps = normal_maps_hidden_states[:, text_seq_length:]
        depth_maps = depth_maps_hidden_states[:, text_seq_length:]
        seg_masks = seg_masks_hidden_states[:, text_seq_length:]
        hand_keypoints = hand_keypoints_hidden_states[:, text_seq_length:]

        # Combine hidden states and tracking maps initially [B, S, C]
        condition_maps = torch.cat([tracking_maps, normal_maps, depth_maps, seg_masks, hand_keypoints], dim=2)
        combined = hidden_states.repeat(1, 1, 5) + condition_maps
        condition_maps = self.initial_combine_linear(combined)
        ...
        for i in range(len(self.transformer_blocks)):
            ...
            if i < len(self.transformer_blocks_copy):
                if self.training and self.gradient_checkpointing:
                    ...
                    condition_maps, _ = torch.utils.checkpoint.checkpoint(
                    ...
                    condition_maps, _ = self.transformer_blocks_copy[i](
                        hidden_states=condition_maps,
                        encoder_hidden_states=encoder_hidden_states,
                        temb=emb,
                        image_rotary_emb=image_rotary_emb,
                    )
                ...
                condition_maps = self.combine_linears[i](condition_maps)
                hidden_states = hidden_states + condition_maps
...
class CogVideoXPipelineCombination(CogVideoXPipeline, DiffusionPipeline):
    ...
    def __init__(
        ...
        transformer: CogVideoXTransformer3DModelCombination,
        scheduler: Union[CogVideoXDDIMScheduler, CogVideoXDPMScheduler],
    ):
        super().__init__(tokenizer, text_encoder, vae, transformer, scheduler)
        
        if not isinstance(self.transformer, CogVideoXTransformer3DModelCombination):
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                normal_maps_latent = torch.cat([normal_maps] * 2) if do_classifier_free_guidance else normal_maps
                depth_maps_latent = torch.cat([depth_maps] * 2) if do_classifier_free_guidance else depth_maps
                seg_masks_latent = torch.cat([seg_masks] * 2) if do_classifier_free_guidance else seg_masks
                hand_keypoints_latent = torch.cat([hand_keypoints] * 2) if do_classifier_free_guidance else hand_keypoints
                ...
                noise_pred = self.transformer(
                    ...
                    normal_maps=normal_maps_latent,
                    depth_maps=depth_maps_latent,
                    seg_masks=seg_masks_latent,
                    hand_keypoints=hand_keypoints_latent,
                    return_dict=False,
                )[0]
...
class CogVideoXImageToVideoPipelineCombination(CogVideoXImageToVideoPipeline, DiffusionPipeline):
    ...
    def __call__(
        ...
        normal_maps: Optional[torch.Tensor] = None,
        normal_image: Optional[torch.Tensor] = None,
        depth_maps: Optional[torch.Tensor] = None,
        depth_image: Optional[torch.Tensor] = None,
        seg_masks: Optional[torch.Tensor] = None,
        seg_image: Optional[torch.Tensor] = None,
        hand_keypoints: Optional[torch.Tensor] = None,
        hand_keypoints_image: Optional[torch.Tensor] = None
    ) -> Union[CogVideoXPipelineOutput, Tuple]:
        ...
        normal_image = self.video_processor.preprocess(normal_image, height=height, width=width).to(
            device, dtype=prompt_embeds.dtype
        )

        depth_image = self.video_processor.preprocess(depth_image, height=height, width=width).to(
            device, dtype=prompt_embeds.dtype
        )

        seg_image = self.video_processor.preprocess(seg_image, height=height, width=width).to(
            device, dtype=prompt_embeds.dtype
        )

        hand_keypoints_image = self.video_processor.preprocess(hand_keypoints_image, height=height, width=width).to(
            device, dtype=prompt_embeds.dtype
        )
        ...
        _, normal_image_latents = self.prepare_latents(
            normal_image,
            batch_size * num_videos_per_prompt,
            latent_channels,
            num_frames,
        ...
        _, seg_image_latents = self.prepare_latents(
            seg_image,
            batch_size * num_videos_per_prompt,
            latent_channels,
            num_frames,
        ...
        _, hand_keypoints_image_latents = self.prepare_latents(
            hand_keypoints_image,
            batch_size * num_videos_per_prompt,
            latent_channels,
            num_frames,
            height,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                # Handle condition maps [B, F, C, H, W]
                ...
                if normal_maps is not None:
                    latents_normal_image = torch.cat([normal_image_latents] * 2) if do_classifier_free_guidance else normal_image_latents
                    normal_maps_input = torch.cat([normal_maps] * 2) if do_classifier_free_guidance else normal_maps
                    normal_maps_input = torch.cat([normal_maps_input, latents_normal_image], dim=2)
                    del latents_normal_image
                else:
                    normal_maps_input = None
                
                if depth_maps is not None:
                    latents_depth_image = torch.cat([depth_image_latents] * 2) if do_classifier_free_guidance else depth_image_latents
                    depth_maps_input = torch.cat([depth_maps] * 2) if do_classifier_free_guidance else depth_maps
                    depth_maps_input = torch.cat([depth_maps_input, latents_depth_image], dim=2)
                    del latents_depth_image
                else:
                    depth_maps_input = None

                if seg_masks is not None:
                    latents_seg_image = torch.cat([seg_image_latents] * 2) if do_classifier_free_guidance else seg_image_latents
                    seg_masks_input = torch.cat([seg_masks] * 2) if do_classifier_free_guidance else seg_masks
                    seg_masks_input = torch.cat([seg_masks_input, latents_seg_image], dim=2)
                    del latents_seg_image
                else:
                    seg_masks_input = None
                
                if hand_keypoints is not None:
                    latents_hand_keypoints_image = torch.cat([hand_keypoints_image_latents] * 2) if do_classifier_free_guidance else hand_keypoints_image_latents
                    hand_keypoints_input = torch.cat([hand_keypoints] * 2) if do_classifier_free_guidance else hand_keypoints
                    hand_keypoints_input = torch.cat([hand_keypoints_input, latents_hand_keypoints_image], dim=2)
                    del latents_hand_keypoints_image
                else:
                    hand_keypoints_input = None
                ...
                noise_pred = self.transformer(
                    ...
                    normal_maps=normal_maps_input,
                    depth_maps=depth_maps_input,
                    seg_masks=seg_masks_input,
                    hand_keypoints=hand_keypoints_input,
                    return_dict=False,
                )[0]
...
class CogVideoXVideoToVideoPipelineCombination(CogVideoXVideoToVideoPipeline, DiffusionPipeline):
